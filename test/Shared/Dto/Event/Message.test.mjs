...
const fact = await container.get('TeqFw_Core_Shared_App_Event_Message$');

describe('TeqFw_Core_Shared_App_Event_Message', function () {

    it('DTO has right namespace', async () => {
        const dto = fact.createDto();
        assert.strictEqual(dto.constructor.namespace, 'TeqFw_Core_Shared_App_Event_Message.Dto');
    });

    it('can create DTO and clone input data', async () => {
        const input = {data: {name: 'data'}, meta: {name: 'eventName'}};
        /** @type {TeqFw_Core_Shared_App_Event_Message.Dto} */
